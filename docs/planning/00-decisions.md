...
Side = Aside Context Awareness(이하 "Aside CA")를 macOS에서 독립 재현하는 로컬 우선 수동 기억 계층.
원본 요구사항: `~/.aside/u/0/sessions/2026-09-23_IWygSZhom4nLN3Kl/artifacts/context-awareness-prd.md` (이하 "원본 PRD").

신뢰 등급: `[verified]` 원본 PRD가 바이너리에서 직접 읽은 값 · `[measured]` 이 프로젝트에서 2026-09-23~24 실측 · `[decided]` 사용자 결정 · `[design]` 이 spec에서 새로 정한 설계 · `[spike]` 구현 전 실험으로 확정해야 함.
...
- **근거** `[measured]`
  ...
  - Aside CA ledger 실경로는 `~/.aside/u/0/context-awareness/ledger.db`이고 테이블 4개가 원본 PRD와 일치한다. 그러나 행 수는 전부 0이다.
  - Aside daemon은 Bun 단일 바이너리라 fork할 수 없고, 기능 자체가 Max 플랜으로 게이트된다.
- **결과**: CA의 본체(네이티브 관찰·입력·OCR·ledger·요약 큐)는 전부 Side가 직접 구현한다.

## ADR-002 스코프 = 원본 PRD 1:1, macOS 전용 `[decided]`

- 원본 PRD FR-1~FR-10과 §11 상수표를 재현 대상으로 삼는다.
- **제외**: Windows(`win32`, `win_uia` source, Windows 전용 문구)와 플랜 게이트(§4 `CONTEXT_AWARENESS_ALLOWED_PLANS`). 개인용 앱이라 게이트할 플랜이 없다.
- **의도적 편차**는 각 ADR과 `01-prd.md` §4 편차표에 모아 둔다.
...
- 원본의 `@moss-js/moss-core`는 `license = Proprietary`라 사용할 수 없다 `[measured: npm view 0.29.0]`.
...
- 설정 UI의 섹션과 문구는 원본 FR-7과 1:1로 맞춘다.

## ADR-007 프로세스 부모 관계 반전: App → daemon `[design]`

- 원본은 daemon이 native helper를 spawn한다(health의 `responsibleSelf`로 보아 TCC responsibility를 disclaim하는 것으로 추정).
...
- 프로토콜은 원본과 같은 JSON-lines(`health|event|result|protocol-error`)를 쓰고 방향만 바뀐다(`02-architecture.md` §3).

## ADR-008 저장 스키마 = Aside DDL 1:1 + 민감 컬럼 봉인 `[decided]`

- 테이블·컬럼·인덱스는 Aside ledger의 실제 DDL을 그대로 쓴다 `[measured: sqlite_master 덤프]`. PK는 TEXT(ULID)다.
- **편차**: `window_title`, `url`, `target`, `payload`, `blobs.content`는 AES-256-GCM으로 봉인한 값을 저장한다. 필터용인 `occurred_at, source, kind, app_name, bundle_id, domain, session_id`만 평문으로 둔다.
...
- 원본 health의 `screenOcrLanguages` 필드와 대응한다. 외부 의존성(brew tesseract)이 없어진다.
...
- AX 트리가 `EMPTY_TREE_BYTES(2048)` 미만이면 원본처럼 `chromeOnly`로 health에 보고하고 OCR 폴백 후보가 된다.
...
- 원본은 `/^gpt-\d+(\.\d+)*-luna(-\d{8})?$/`와 `/^claude-haiku-/`에 `thinkingLevel='high'`를 하드코딩한다.
- Side는 `summary.modelOverrides: [{match: <regex>, reasoningEffort, fastMode}]`로 일반화하고, 원본 두 규칙을 기본값으로 넣는다.
